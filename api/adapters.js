...
module.exports = async function adapterRoutes(fastify) {
  ...
  fastify.get('/api/adapters', async (request, reply) => {
    try {
      ...
      if (platform === 'windows') {
        return reply.send({ platform: 'windows', adapters: await getWindowsAdapters(), supportsExternal: true });
      }
      if (platform === 'android') {
        return reply.send({ platform: 'android', adapters: await getAndroidUSBAdapters(), supportsUSBOTG: true, supportsRoot: true });
      }
